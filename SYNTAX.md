...
Mire uses `load` to declare dependencies and `use` for importing. Dependencies
are declared in `owl.toml`.
...
### `use` — output and import

```mire
use dasu("hello")         # output expression (built-in)
use strings.upper("hi")   # call a namespaced function
```
...
| `task` | `spawn`, `join`, `ready`, `failed`, `value`, `error` |
